...
int main(int argc, char** argv){
  ...
  cv::Mat histR, histG, histB,previous_histR,hist_difference;
  int nbins = 64;
  float range[] = {0, 255};
  ...
  int key;
  double max_hist_diff = 0 , max_red_hist = 0;
  ...
  cv::calcHist(&planes[0], 1, 0, cv::Mat(), previous_histR, 1,
                  &nbins, &histrange,
                  uniform, acummulate);
  ...
  while(1){
    ...
    /*
    std::cout<<"hist_difference = "<<std::endl<<hist_difference<<std::endl;
    std::cout<<"histR = "<<std::endl<<histR<<std::endl;
    std::cout<<"previous_histR = "<<std::endl<<previous_histR<<std::endl;
    std::cout<<"max_hist_diff = "<<std::endl<<max_hist_diff<<std::endl<<"max_red_hist = "<<std::endl<<max_red_hist<<std::endl;
    */

    //std::cout<<max_hist_diff/max_red_hist<<std::endl;
